...
args = ArgumentParser('python Z_score_tables.py', description="""Welcome to Z_score_tables.py. This program has been designed to scale rows of data.
				...
				(often Ensembl ID or gene names for genomic data). The output file will contain scaled data, meaning that
				each value will represent the Z-score for its given row.
				Example usage: python Z_score_tables.py --input_csv RCPM_table.csv --output_file Scaled_RCPM_table.csv""")
...
args.add_argument(
	...
	option, the name of the input file with "Z_score" added on the end will be used.""",
	default = None
)
...
if args.input_csv == None:
	...
	print()
	print("""		Welcome to Z_score_tables.py. This program has been designed to scale rows of data.
		The input must be in a comma delimited (csv) format. The header line must contain the names of
		the columns (usually sample names). The first column must contain the names for each row
	...
	print("		Example usage: python Z_score_tables.py --input_csv RCPM_table.csv --output_file Z_score_RCPM_table.csv")
	print()
	sys.exit(0)
...
if output_file == None:
	output_file = splitext(input_file)[0]+"_Z_score.csv"

input_data = pd.read_csv(input_file, index_col=0)
z_table = input_data.apply(zscore, axis = 1)
z_table = pd.DataFrame(z_table.to_list())
z_table.columns = input_data.columns
z_table['names'] = input_data.index
z_table = z_table.set_index('names')
z_table.to_csv(output_file)
